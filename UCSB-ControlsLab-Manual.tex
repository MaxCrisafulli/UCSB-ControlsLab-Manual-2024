...
%%%%%%%%%%% TITLEPAGE %%%%%%%%%%%%%
\begin{center}
  \Huge{\textbf{UCSB ECE Controls Lab TA Manual}}
  \vspace{0.5cm}
  \\ \textbf{By \large{Max Crisafulli} \& \large{Zeki Duman}}
  \vspace{0.2cm}
  \\ September 2024 (Updated April 2026)
...
This document is to act as a guide for TAs in setting up the Quanser inverted pendulum cart hardware in the UCSB ECE Controls Lab (Harold Frank Hall 3120A). This guide is an update of the existing manual:
\[ \text{ \url{https://github.com/justinpearson/UCSB-Quanser-Inverted-Pendulum-Lab-Manual}}\]
This updated guide will live and (ideally) be maintained at 
...
\begin{itemize}
  ...
  \item Contact the IT people for computer problems, \textsc{Matlab} issues, and getting TA/student login access:
...
\begin{enumerate}
  \item You must use \texttt{\textsc{Matlab} R2023a} \textit{only}. The other versions will not work. Start \textsc{Simulink} and create a new model.
  
  %\item In the blank model window, choose \texttt{Simulation $\baro$ Mode $\baro$ External}
  
  \item Choose \texttt{Modeling $\baro$ Model Settings}
  
  \item Set the following parameters
  \begin{enumerate}
    \item Select \texttt{Solver} from the tree at the left
    \item Stop time: \texttt{inf} - or whatever $T_f$ is desired (in seconds)
    \item Solver options, solver type: \texttt{Fixed-Step}
    \item Solver options, solver: \texttt{discrete} if there are no continuous states. Select a solver like \texttt{ode4} if you are using continuous states (like a continuous transfer function)
    \item Solver options, Fixed-step size (under Solver details): \texttt{0.001}
    ...
    \end{figure}
    \item Choose \texttt{Code Generation} in the tree on the left.
    \item Ensure the System target file is \texttt{quarc\_win64.tlc}. If not, select the \texttt{Browse} button and select it. The settings should look as follows. 
    \begin{figure}[H]
      \centering 
    ...
    \end{figure}
  \end{enumerate}
  \item Select OK and return to the \textsc{Simulink} model.
  \item In the \textsc{Simulink} Library Browser, find the \texttt{HIL Initialize} block. It is located in \texttt{QuaRC Targets $\baro$ Data Acquisition $\baro$ Generic $\baro$ Configuration}. You can also search by double clicking the model background. Add the block into your model.
  \begin{enumerate}
  ...
  \end{enumerate}
  \item Double click on the \texttt{HIL Initialize} block in your model. Change the Board type to \texttt{q4}. Make sure the Board Identifier is \texttt{0}. Click Apply and Ok out of the block.
  \begin{figure}[H]
    \centering 
  ...
  \end{figure}
  \item Add a \texttt{HIL Write Analog} block and a \texttt{HIL Read Encoder} block to your model. They are in \texttt{QuaRC $\baro$ Data Acquisition $\baro$ Generic $\baro$ Immediate I/O}. Double click on them and set their \texttt{Sample Time} to 0.001.
  \item Connect these blocks as depicted in Figure \ref{fig:simple_model}. In this example, we place a simple constant output and a display for the encoder. \textit{For the purposes of this example, do not output more than $\pm$2.5 volts to the cart to prevent uncontrolled motion.}
  \begin{figure}[H]
    \centering 
  ...
  \end{figure}
  \item When your model is ready, save it. From the \texttt{QuaRC} menu, select \texttt{Build}. \textsc{Matlab} will compile your \textsc{Simulink} model into native machine code. You can check the status of the build by clicking \texttt{View diagnostics} at the bottom of the \textsc{Simulink} window. \textit{Every time you structurally change your model (add new blocks, rewire existing blocks) you will need to rebuild it or an error will occur.} 
  \begin{enumerate}
  ...
  \item When the build is completed, go back to the model and choose \texttt{QuaRC $\baro$ Monitor \& Tune}. This downloads the compiled model to the QuaRC execution program and then model will run. The \texttt{Stop} button in the Quarc menu will stop the model as normal.
  \begin{enumerate}
    %\item \textit{Tip}: You can also click Connect to Target, followed by the \textsc{Simulink} Play Button. This is the same as using the QuaRC menu.
    \item \textit{Troubleshooting Tip}: If the \textsc{Simulink} time is not running (at the bottom of the model window), the QuaRC execution environment may have crashed. Try logging out and logging back in or rebooting the computer.
    \item \textit{Tip}: Suppose your model is running and \textsc{Matlab} crashes. The model will continue to run. First, ensure the hardware is not damaged by lifting it off the track and then switching off the Amplifier.
    
    % After the hardware is safe, stop the model by checking for the Q icon in the system tray (by the clock in the lower right). If the Q icon is not there, start it from Start Menu $\rightarrow$ All Programs $\rightarrow$ Quanser QuaRC $\rightarrow$ Monitor. Right click the Q and choose \texttt{Stop all}.
  \end{enumerate}
...
\section{Demo Model}
\begin{enumerate}
  \item Generate the \textsc{Simulink} diagram shown in Figure \ref{fig:Simulink_loopback}. The Sine wave should have a relatively low magnitude of around 2 and a frequency of around 1 Hz.
  \begin{figure}[H]
    \centering 
    \includegraphics[width=.75\textwidth]{Figures/fig10_loopback.png}
    \caption{\textsc{Simulink} Loopback System}
    \label{fig:Simulink_loopback}
  \end{figure} 
  \item The output is an integer corresponding to counts. There is certain number of counts/revolution of the optical encoder's shaft. The gain seen in the figure is the `conversion factor for encoder reading'. It should be $0.1524/5000$ for meters or $6/5000$ for inches, ask your TA to make sure. 
  \item After creating the system diagram and saving it, build the model following steps written in Section \ref{subsubsec:hardware_setup}. This generates the real-time code for the diagram. Wait until the compilation is complete. The \textsc{Simulink} window diagnostics panel displays the progress of the code generation task and, when it is complete, the following message appears:
  \begin{figure}[H]
    \centering 
...
\newpage
\includepdf[pages=-]{SetupSlides.pdf}
